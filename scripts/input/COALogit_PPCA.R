COALogit_PPCA <- function(refgdx,recovery,size,PPCA_pol,oecd,nonoecd,outputfolder,rev,title,fin_pol,uncertainty="90_CI") {
  ...
  # setConfig(cachefolder = paste0("/p/projects/rd3mod/inputdata/cache/rev",rev))
  # setConfig(cachefolder = paste0("/p/projects/rd3mod/inputdata/cache/rev","5.964"))
  setConfig(cachefolder = "/p/projects/rd3mod/inputdata/cache/SB_GCPT")
  ...
  #SSP2 GDP per capita
  gdppc <- calcOutput("GDPpc",aggregate=F) * 1e-3
  gdppc <- gdppc[,c(2015,2025,2045),"SSP2"]
  # setConfig(forcecache=F)

  for (ii in 1:length(phase)) {
    for (jj in 1:length(subpolicy)) {
      # out[ii] <- calcOutput(type="PPCA",phase[ii],size,policy,subpolicy[jj],recovery,run,file=paste0(paste("f47",phase[ii],recovery,policy,size,sep="_"),".cs4r"))
      ...
      #SSP2 GDP PPP
      gdpPPPC <- calcOutput("GDPpppFuture",GDPpppFuture="SSP_completed",aggregate = F,years = getYears(rundata)[which(getYears(rundata)>="y2015")])[,,"gdp_SSP2"]
      gdpPPPR <- toolAggregate(gdpPPPC,map,NULL)
      # setConfig(forcecache = F)
      # Use GDP * population to disaggregate future regional electricity demand
      # prodC <- popC * gdpPPPC
      # prodR <- popR * gdpPPPR
      # prodR <- toolAggregate(prodR,map,NULL)
      # 
      # popGDPshare_c <- prodC / prodR
      # 
      ...
      # Cases with no coal capacity constraint in 2025 due to Covid or FinEx
      if (recovery=="none" | grepl("REdirect",fin_pol)) {
      ...
      # Cases with fixed 2025 capacity due to Covid recovery or FinEx scenario
        ...
        if (fin_pol=="G20_coal") {
          cap_2025_c <- readGCPT_finEx(subtype = "G20_FinEx_2020")
        }else if (fin_pol=="none") {
      ...
      if (grepl("current",size)) {
        ...
        # s3 <- read.csv(paste0(getConfig("sourcefolder"),"/PPCA/PPCA_status_2020.csv"),stringsAsFactors = F,sep = ",")
        s3 <- read.csv(paste0(getConfig("sourcefolder"),"/PPCA/PPCA_status_SSP2_2021.csv"),stringsAsFactors = F,sep = ",")
      ...
      for (reg in unique(map$RegionCode)) {
        ...
        logit_coalgen_2030_c[reg_oecd_members,,] <- 1e-3
        ...
        if (length(reg_oecd_members) & length(reg_nonzero_freeriders)) {
          ...
          while (length(reg_excess) & length(reg_nonzero_freeriders)) {
            # excess <- dimSums(logit_coalgen_2030_c[reg_excess,,] - 
            #                                         (leakage_allowance * 
            #                                           replace_non_finite(coalgen_2030_c[reg_excess,"y2030",]/
            #                                                                 totalgen_2030_c[reg_excess,"y2030",],replace=0)), dim=1)   
            ...
            logit_coalgen_2030_c[reg_excess,,] <- leakage_allowance * coalgen_2030_c[reg_excess,"y2030",]
                            print(paste(reg,"coalgen_2030_c OK..."))
            ...
            if (length(reg_nonzero_freeriders)) {
              ...
              logit_coalgen_2030_c[reg_nonzero_freeriders,,] <- logit_coalgen_2030_c[reg_nonzero_freeriders,,] + excess * reg_normalized_pow_dem
                                        print(paste(reg,"totalgen_2030_c OK..."))
      ...
      if (grepl("demand",PPCA_pol)) {
        ...
        if (grepl("steel",subpolicy[jj])) {
          ...
          coalemi_2030_R <- coalemi_2030_R[-which(getRegions(coalemi_2030_R)=="GLO"),,"Emi|CO2|Energy|Demand|Solids|After IndustryCCS (Mt CO2/yr)"] * 
            (coalemi_2030_R[-which(getRegions(coalemi_2030_R)=="GLO"),,"PE|Coal|Solids"] / 
            (coalemi_2030_R[-which(getRegions(coalemi_2030_R)=="GLO"),,"PE|Coal|Solids"] + coalemi_2030_R[-which(getRegions(coalemi_2030_R)=="GLO"),,"PE|Biomass|Solids"])) - 
      ...
      print("logit_coalShare_2045_c OK...")
      ...
      coalgen_2045_c <- coalShare_2045_c * totalgen_2045_c
                            print("totalgen_2030_c, totalgen_2030_c, totalgen_2030_R OK...")
      ...
      if (grepl("power",PPCA_pol)) {
        ...
        if (grepl("steel",subpolicy[jj])) {
          ...
          coalemi_2050_R <- coalemi_2050_R[-which(getRegions(coalemi_2050_R)=="GLO"),,"Emi|CO2|Energy|Demand|Solids|After IndustryCCS (Mt CO2/yr)"] *
            (coalemi_2050_R[-which(getRegions(coalemi_2050_R)=="GLO"),,"PE|Coal|Solids"] / 
            (coalemi_2050_R[-which(getRegions(coalemi_2050_R)=="GLO"),,"PE|Coal|Solids"] + coalemi_2050_R[-which(getRegions(coalemi_2050_R)=="GLO"),,"PE|Biomass|Solids"])) - 
  ...
  # print(dimSums(toolAggregate(coalgen_2050_c[,getYears(coalgen_2050_c)>="y2050",],map,NULL),dim=2) /
      ...
      # print(toolAggregate(out,rel=map,weight=weight_out))
      ...
      #   # if (grepl("power",PPCA_pol))  weight <- totalgen_2050_c
      #   # else if (grepl("demand",PPCA_pol))  weight <- totalemi_2050_c
      # }else {
      #   # if (grepl("power",PPCA_pol))  weight <- totalgen_2030_c
      #   # else if (grepl("demand",PPCA_pol))  weight <- totalemi_2030_c
      # }
  ...
  #   if (grepl("quartiles",uncertainty))  p <- c(.25, 0.5, 0.75)
  #   else if (grepl("thirds",uncertainty))  p <- c(.33, 0.5, 0.67)
  #   else if (grepl("90_CI",uncertainty))  p <- c(0.05, 0.5, 0.95)
  #   
  #   lts <- c(3, 2, 4)
  #   
  #   names(lts) <- as.character(p)
  # 
  #   s4 <- data.frame(country=map$X,
  #                    share=as.numeric(ifelse(grepl("non",phase),coalShare_2045_c[,timestep,], coalShare_2025_c[,timestep,])),
  #                    gdp=as.numeric(gdppc[,timestep,]),
  #                    Capacity=as.numeric(ifelse(grepl("non",phase),logit_coalcap_2045_c[,timestep,], cap_2025_c[,timestep,])),
  #                    OECD=as.character(oecd_map),
  #                    PPCA=PPCAmap$RegionCode,
  #                    Region=map$RegionCode)
  #   
  #   s4$share[which(s4$share>1)] <- max(s4$share[which(s4$share<1)])
  #   
  #   s4 <- s4 %>% 
  #     mutate(Probability=predict(logit2,newdata = data.frame(Coal.Share=share,GDP.PC=gdp),type = "response")) %>%
  ...
  #     s4 <- s4 %>% mutate(nudge = ifelse(country=="MAR",-0.02,ifelse(country=="CHN",0.065,0.025+Capacity/1.5e4)))
  #   else if (grepl("non",phase))
  #     s4 <- s4 %>% mutate(nudge = ifelse(country=="CHN",0.065,0.025+Capacity/1.5e4))
  #   
  #   # mutate(nudge = ifelse(Capacity>250,0.055,0))
  #   # Country = ifelse(OECD=="OECD", str_c(country, "*"), country))
  #   
  #   s5 <- filter(s4,share>=1e-2)
  #   
  #   if (grepl("non",phase,ignore.case=T)) {
  #     s5 <- filter(s5,!(PPCA=="PPCA" & OECD=="OECD"))
  #   }  
  #   
  #   s5 <- s5 %>% 
  ...
  #   # colnames(s5)[which(grepl("cap_",colnames(s5)))] <- paste0("GW Capacity (",gsub("cap_","",colnames(s5)[which(grepl("cap_",colnames(s5)))]),")")
  #   
  ...
  #     geom_point(data = s5,aes(x = share, y = gdp, color = OECD, size = Capacity),shape=16) +
  ...
  #     new_scale_color() +
  #     # Add black contours to all bubbles
  #     # geom_point(data = s5, 
  #     #            aes(x = share, y = gdp, size = Capacity), shape = 1, color = "black", stroke = 0.5) +
  #     ##Special treatment of countries 'overshadowed' by bigger ones
  #     # geom_point(data = filter(s5, country %in% c("KZ")), 
  #     #            aes(x = share, y = gdp, size = Capacity, color = PPCA,)) +
  #     # geom_point(data = filter(s5, country %in% c("KZ")), 
  #     #            aes(x = share, y = gdp, size = Capacity), shape = 1, color = "black", stroke = 0.5) +
  #     
  #     #Labels
  #     geom_text_repel(data = s5, 
  #                     aes(x = share, y = gdp, label = country, fontface = Face, color = PPCA), 
  #                     size = 5.2, segment.size = 0.3,segment.alpha = 0.25,force = 5,nudge_x = s5$nudge,max.overlaps = 80) +
  #     
  ...
  #     
  #     ##Special treatment of countries 'overshadowed' by bigger ones
  #     geom_point(data = filter(s5, ifelse(phase=="OECD" & recovery=="Neutral", country %in% c("VNM"),
  #                                         country %in% "")),
  ...
  #           plot.title = element_text(face="bold",size=2.5*10,hjust=0.5),
  ...
  #   
  #   
  #   ggsave(filename=paste0(outputfolder,paste("DFS",phase,PPCA_pol,size,recovery,fin_pol,sep="_"),".png"),width=6.3*3,height=4.63*3,units="in",dpi="retina")
  # }
}
