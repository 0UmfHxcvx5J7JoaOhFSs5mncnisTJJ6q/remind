...
ces_subtree <- read.gdx(gdx, 'cesOut2cesIn_below',
                        colNames = c('pf.out', 'pf.in'))

ppf <- read.gdx(gdxName = gdx, requestList.name = 'ppf')$all_in
...
CES.cal.report = CES.cal.report %>%
    filter(iteration %in% c("target", "origin", itr_num))
...
## sort industry/subsector pf ----
x <- c()
for (ipf in c('ue_cement', 'ue_chemicals', 'ue_steel_primary',
              'ue_steel_secondary', 'ue_otherInd')) {
    if (ipf %in% .pf$structure) {
        x <- c(x, ipf,
               ces_subtree %>%
                   filter(ipf == pf.out,
                          !pf.in %in% ppf) %>%
...
.pf$structure <- c(setdiff(.pf$structure, x), x)
...
target_quantities <- CES.cal.report %>%
    filter('target' == iteration,
           'quantity' == variable,
           0 != value) %>%
    pull('pf') %>%
    unique()

try(grid.table(CES.cal.report %>%
                   filter(
                       pf %in% target_quantities) %>%
                   quant_outliers(threshold_quant),
               rows = NULL))
